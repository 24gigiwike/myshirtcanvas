...
export function SignoutStudio() {
  ...
  const handleStampCreate = (point: THREE.Vector3, normal: THREE.Vector3) => {
    ...
    context.font = "bold 48px 'Brush Script MT', 'Comic Sans MS', cursive"
    context.textAlign = 'center'
    context.textBaseline = 'middle'
    context.fillText(stampMessage.trim(), 256, 128, 480)
    setStamps((current) => [...current, { id: Date.now(), textImage: textCanvas.toDataURL('image/png'), position: [point.x, point.y, point.z], rotation: [rotation.x, rotation.y, rotation.z], scale: [1.5, 0.75, 1.5] }])
  }
