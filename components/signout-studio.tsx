...
function AboutPage({ onBack }: { onBack: () => void }) {
  return <main className="min-h-dvh scroll-smooth bg-[#fdfbf7] text-[#183b38]"><header className="mx-auto flex max-w-6xl items-center justify-between px-6 py-8"><div><p className="text-xs font-bold uppercase tracking-[0.3em] text-[#0e8f89]">Get to know me</p><h1 className="mt-2 text-4xl font-black tracking-tight md:text-6xl">A few chapters of Great.</h1></div><button type="button" onClick={onBack} className="rounded-full border border-[#183b38]/15 px-5 py-2 text-sm font-bold transition hover:bg-[#183b38] hover:text-white">About the studio</button></header><div className="mx-auto max-w-6xl px-6 pb-16">{ABOUT_STORY.map((story, index) => <article key={story.title} className={`grid items-center gap-8 border-t border-[#183b38]/10 py-14 md:grid-cols-2 md:gap-16 md:py-24 ${index % 2 ? 'md:[&>div:first-child]:order-2' : ''}`}><div className="overflow-hidden rounded-[2rem] bg-[#e8eee7] shadow-[0_20px_60px_rgba(24,59,56,0.12)]"><img src={story.image} alt={`${story.title} chapter`} className="aspect-[4/3] w-full object-cover" /></div><div className="max-w-xl"><p className="text-sm font-bold tracking-[0.2em] text-[#0e8f89]">0{index + 1} / 06</p><h2 className="mt-3 text-4xl font-black tracking-tight md:text-5xl">{story.title}</h2><p className="mt-6 text-lg leading-8 text-[#41635f]">{story.text}</p></div></article>)}</div><footer className="mx-auto flex max-w-6xl flex-col gap-3 border-t border-[#183b38]/10 px-6 py-10 sm:flex-row"><button type="button" onClick={onBack} className="rounded-full bg-[#0e8f89] px-6 py-3 text-sm font-bold text-white transition hover:bg-[#087f7b]">Back to Home</button><a href="https://webdesignking.online" target="_blank" rel="noreferrer" className="rounded-full border border-[#183b38]/20 px-6 py-3 text-center text-sm font-bold transition hover:bg-[#183b38] hover:text-white">See My Portfolio</a></footer></main>
}
