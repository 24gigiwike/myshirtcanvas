...
export function SignoutStudio() {
  ...
  return <main className="relative h-dvh w-full overflow-hidden bg-[#0eb0ab]"><div className="absolute inset-0"><Scene color={active.value} drawing={drawing} setDrawing={setDrawing} zoomLevel={zoomLevel} groupRef={groupRef} /></div><aside className="absolute bottom-5 left-1/2 z-10 flex max-w-[calc(100vw-1rem)] flex-wrap items-center justify-center gap-3 rounded-3xl border border-white/30 bg-[#087f7b]/70 p-3 shadow-2xl backdrop-blur-xl md:bottom-auto md:left-5 md:top-1/2 md:-translate-y-1/2 md:translate-x-0 md:flex-col"><div className={`${markerTrayOpen ? 'flex' : 'hidden'} absolute bottom-full left-0 mb-2 w-full items-center justify-center gap-1 rounded-2xl border border-white/25 bg-[#087f7b]/90 p-2 shadow-xl backdrop-blur-xl md:hidden`}>{COLORS.map((item) => <Marker key={item.name} item={item} active={active.name === item.name} onClick={() => chooseMarker(item)} />)}</div><div className="flex items-center gap-1 md:flex-col"><div className="flex items-center md:hidden"><Marker item={active} active onClick={() => setMarkerTrayOpen((open) => !open)} /><button type="button" aria-label="Toggle marker colors" aria-expanded={markerTrayOpen} onClick={() => setMarkerTrayOpen((open) => !open)} className="px-1 text-xs text-white">⌄</button></div><div className="hidden md:flex md:flex-col md:gap-1">{COLORS.map((item) => <Marker key={item.name} item={item} active={active.name === item.name} onClick={() => chooseMarker(item)} />)}</div></div><div className="h-px w-8 bg-white/30" /><div className="grid w-[7.5rem] grid-cols-3 gap-1 md:w-auto md:grid-cols-5"><button type="button" aria-label="Rotate left" onClick={() => rotate('y', -0.22)} className="studio-control"><ArrowLeft size={15} /></button><button type="button" aria-label="Rotate right" onClick={() => rotate('y', 0.22)} className="studio-control"><ArrowRight size={15} /></button><button type="button" aria-label="Rotate up" onClick={() => rotate('x', -0.16)} className="studio-control"><ArrowUp size={15} /></button><button type="button" aria-label="Rotate down" onClick={() => rotate('x', 0.16)} className="studio-control"><ArrowDown size={15} /></button><button type="button" aria-label="Reset rotation" onClick={() => groupRef.current?.rotation.set(0.02, 0, 0)} className="studio-control"><RotateCcw size={14} /></button></div><div className="flex items-center gap-1 border-l border-white/20 pl-2"><button type="button" aria-label="Zoom out" onClick={() => setZoomLevel((v) => Math.min(8, v + 0.5))} className="studio-control text-lg">−</button><button type="button" aria-label="Zoom in" onClick={() => setZoomLevel((v) => Math.max(1.5, v - 0.5))} className="studio-control text-lg">+</button></div></aside></main> }
