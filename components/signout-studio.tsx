...
function Scene({ color, drawing, setDrawing, zoomLevel, onZoomIn, onZoomOut }: { color: string; drawing: boolean; setDrawing: (v: boolean) => void; zoomLevel: number; onZoomIn: () => void; onZoomOut: () => void }) {
  ...
  return <div className="relative h-full w-full">
    <Canvas shadows camera={{ position: [0, 0, zoomLevel], fov: 45 }} onPointerMissed={() => setDrawing(false)}>
    ...
    <div className="absolute bottom-5 right-5 flex items-center gap-1 rounded-2xl border border-foreground/10 bg-background/80 p-1.5 shadow-lg backdrop-blur" aria-label="Zoom controls">
      <button type="button" aria-label="Zoom out" onClick={onZoomOut} className="control-button text-lg font-semibold" disabled={zoomLevel >= 8}>−</button>
      <span className="min-w-10 text-center font-mono text-[10px] text-muted-foreground">{zoomLevel.toFixed(1)}x</span>
      <button type="button" aria-label="Zoom in" onClick={onZoomIn} className="control-button text-lg font-semibold" disabled={zoomLevel <= 1.5}>+</button>
    </div>
...
export function SignoutStudio() {
  ...
  const zoomIn = () => setZoomLevel((level) => Math.max(1.5, Number((level - 0.5).toFixed(1))))
  const zoomOut = () => setZoomLevel((level) => Math.min(8, Number((level + 0.5).toFixed(1))))
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <div className="mx-auto flex max-w-[1500px] flex-col gap-6 px-5 py-6 md:px-10 lg:flex-row lg:gap-8 lg:py-8">
      <section className="min-w-0 flex-1">
        ...
        <div className="h-[58vh] min-h-[440px] overflow-hidden rounded-[2rem] border border-foreground/10 shadow-[0_20px_60px_-35px_rgba(20,50,30,.35)] md:h-[640px]"><Scene color={active.value} drawing={drawing} setDrawing={setDrawing} zoomLevel={zoomLevel} onZoomIn={zoomIn} onZoomOut={zoomOut} /></div>
      </section>
