...
export function SignoutStudio() {
  ...
  const handleStampCreate = (point: THREE.Vector3, normal: THREE.Vector3) => {
    ...
    context.font = "italic bold 80px 'Nunito', 'Rubik', sans-serif"
    context.textAlign = 'center'
    context.textBaseline = 'middle'
    context.fillText(stampMessage.trim(), 512, 128)
    setStamps((current) => [...current, { id: Date.now(), textImage: textCanvas.toDataURL('image/png'), position: [point.x, point.y, point.z], rotation: [rotation.x, rotation.y, rotation.z], scale: [0.8, 0.2, 0.8] }])
  }
